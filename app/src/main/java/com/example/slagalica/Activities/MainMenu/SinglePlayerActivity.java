...
public class SinglePlayerActivity extends AppCompatActivity {
    ...
    private void referenceElements() {
        listener = new SharedPreferences.OnSharedPreferenceChangeListener() {
            public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
                ...
                switch (key) {
                    case (KorakPoKorakGame.gameName + "Points"):
    ...
    public void startKorakPoKorak(View view) {

        if (!playedGames[0]) {
            playedGames[0] = true;
            checkGameInHistory(KorakPoKorakGame.gameName);
            Intent intent = new Intent(this, KorakPoKorakGame.class);
            startActivity(intent);
        }
    }
